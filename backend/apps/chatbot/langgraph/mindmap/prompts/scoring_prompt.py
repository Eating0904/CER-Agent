...
PROMPT_TEMPLATE = """
# 角色定位
你是 **CER (主張 Claim、證據 Evidence、推論 Reasoning) 評分專家**。
...
2. **[執行邏輯診斷]**: 不要只看關鍵字有沒有出現，要檢查**連線 (Edges)** 的邏輯流動：
    ...
    - **檢查主張 (Claim) - 證據 (Evidence) - 推論 (Reasoning)**：確定推論 (Reasoning) 是否真的能透過該證據 (Evidence) 支持其主張 (Claim)，有無邏輯斷層?
...
# 分數範例
## 名詞解釋
- nodes 為節點清單，清單中每筆資料皆有 id 以及 content，id 的開頭可辨別節點 (Node) 類型，c 表示主張 (Claim)、e 表示證據 (Evidence)、r 表示推論 (Reasoning)，content 則為該節點 (Node) 的內容。
