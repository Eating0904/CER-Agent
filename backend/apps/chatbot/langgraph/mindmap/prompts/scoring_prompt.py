...
PROMPT_TEMPLATE = """
# 角色定位
你是 **CER (主張 Claim、證據 Evidence、推理 Reasoning) 評分專家**。
...
2. **[執行邏輯診斷]**: 不要只看關鍵字有沒有出現，要檢查**連線 (Edges)** 的邏輯流動：
    - **檢查證據 (Evidence) - 主張 (Claim)**：  確定該主張 (Claim) 是否有被證據 (Evidence) 支持?
    - **檢查主張 (Claim) - 證據 (Evidence) - 推理 (Reasoning)**：確定推理 (Reasoning) 是否真的能透過該證據 (Evidence) 支持其主張 (Claim)，有無邏輯斷層?
...
# 分數範例
## 名詞解釋
- nodes 為節點清單，清單中每筆資料皆有 id 以及 content，id 的開頭可辨別節點 (Node) 類型，c 表示主張 (Claim)、e 表示證據 (Evidence)、r 表示推理 (Reasoning)，content 則為該節點 (Node) 的內容。
...
[
  ...
  {{ "id": "reasoning", "content": "推理 (Reasoning)" }},
...
[
  ...
  {{ "id": "reasoning", "content": "推理 (Reasoning)" }},
  {{ "id": "r1", "content": "推理的起點是,文章的環境證據確實很嚴重,顯示畜牧業對氣候有重大威脅。這是我們需要改變飲食的『大理由』。" }},
