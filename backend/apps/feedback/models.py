...
class NodeFeedback(models.Model):
    ...
    alert_title = models.TextField(help_text='Alert 標題（例如：完成了 3 個操作）')
    ...
    metadata = models.JSONField(default=dict, help_text='操作資訊 JSON（包含詳細的節點內容和變更）')
    ...
    def __str__(self):
        return f'Feedback for {self.map.name} - {self.alert_title[:30]}'
